...
DECLARE_WATCHER_DATA( "PrimitiveProcessor" )
DECLARE_COPY_STACK_INFO( true )
DEFINE_CREATE_EDITOR_PROPERTY_STUB

ConverterInfo PrimitiveProcessorInfo;
...
PLUGIN_INIT_FUNC
{
    ...
    WNDCLASS wc = { 0, DefWindowProc, 0, 0, NULL, NULL, NULL, NULL, NULL, L"PrimitiveProcessor" };
    if ( !RegisterClass( &wc ) )
    {
    ...
    HWND hWnd = CreateWindow(
        L"PrimitiveProcessor", L"PrimitiveProcessor",
        WS_OVERLAPPEDWINDOW,
    ...
    INIT_CONVERTER_INFO( PrimitiveProcessorInfo, 
                         PrimitiveProcessor, 
                         ConverterInfo::CACHE_CONVERSION );

    compiler->registerConverter( PrimitiveProcessorInfo );
