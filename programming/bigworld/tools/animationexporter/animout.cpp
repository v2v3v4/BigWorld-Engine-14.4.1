...
void MFExp::ExportAnimKeys(INode* node, int indentLevel) 
{
...
BOOL MFExp::CheckForAnimation(INode* node, BOOL& bPos, BOOL& bRot, BOOL& bScale)
{
...
void MFExp::DumpPosSample(INode* node, int indentLevel) 
{	
	TSTR indent = GetIndent(indentLevel);
...
void MFExp::DumpRotSample(INode* node, int indentLevel) 
{	
	TSTR indent = GetIndent(indentLevel);
...
void MFExp::DumpScaleSample(INode* node, int indentLevel) 
{	
	TSTR indent = GetIndent(indentLevel);
...
void MFExp::DumpPoint3Keys(Control* cont, int indentLevel) 
{
...
// supports key operations. Otherwise we will sample the controller 
// once for each frame to get the value.
void MFExp::DumpFloatKeys(Control* cont, int indentLevel) 
{
	if (!cont)
		return;
...
void MFExp::DumpPosKeys(Control* cont, int indentLevel) 
{
	if (!cont)
		return;
...
void MFExp::DumpRotKeys(Control* cont, int indentLevel) 
{
	if (!cont)
		return;
...
void MFExp::DumpScaleKeys(Control* cont, int indentLevel) 
{
	if (!cont)
		return;
...
MFExp::IsKnownController(Control* cont)
{
	ulong partA, partB;
