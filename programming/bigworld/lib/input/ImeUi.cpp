...
//
// Modifications by BigWorld are marked as such.
//
...
// Uncomment this line for extra logging (BigWorld)
//#define BW_ENABLE_IME_DEBUGGING

#ifdef BW_ENABLE_IME_DEBUGGING
#define BW_IME_DEBUG_MSG DEBUG_MSG
#else
#define BW_IME_DEBUG_MSG(...)
#endif
...
#define _CHS_HKL_GOOGLE ( (HKL)0xE0200804LL ) // Google Pinyin (BigWorld)
#define _CHS_HKL_SOGOU  ( (HKL)0xE0210804LL ) // Sogou (BigWorld)
#define _CHS_HKL_SOGOU_WUBI ( (HKL)0xE0220804LL ) // Sogou Wubi (BigWorld)
...
static UINT (WINAPI*_GetReadingString)(HIMC himc, UINT uReadingBufLen, LPWSTR lpwReadingBuf, PINT pnErrorIndex, BOOL* pfIsVertical, PUINT puMaxReadingLen);
static BOOL (WINAPI*_ShowReadingWindow)(HIMC himc, BOOL bShow);
...
static bool IsUILessMode()
{
	// BigWorld: Centralise this here for special handling of certain 
	//			 IME types which don't play nicely with the usual
	//			 way of doing things...
	return	g_bUILessMode && 
			g_hklCurrent != _CHS_HKL_GOOGLE && 
			g_hklCurrent != _CHS_HKL_SOGOU && 
			g_hklCurrent != _CHS_HKL_SOGOU_WUBI;
}
...
static void ComposeCandidateLine( int index, LPCTSTR pszCandidate )
{
	...
	// Thomas Cowell: We don't want to stick the candidate number into the candidate string here.
	//                It'll be done at a higher level in the GUI scripts.
...
static void DrawCandidateList()
{
	...
	DWORD adwCandWidth[ MAX_CANDLIST ] = {0};
	...
	if (g_bReadingWindow && g_bHorizontalReading)
	{
		...
		TCHAR szTemp[ COUNTOF( g_szReadingString ) ] = {0};
...
//
	case WM_INPUTLANGCHANGE:
		BW_IME_DEBUG_MSG( "WM_INPUTLANGCHANGE (HKL=%x, wParam=%x, lParam=%x)\n", 
			g_hklCurrent, wParam, lParam );
		OnInputLangChange();
		break;
    ...
    case WM_IME_SETCONTEXT:
		...
		//
		BW_IME_DEBUG_MSG( "WM_IME_SETCONTEXT (HKL=%x, wParam=%x, lParam=%x)\n", 
			g_hklCurrent, wParam, lParam );
        lParam = 0;
        break;

    case WM_IME_STARTCOMPOSITION:
		BW_IME_DEBUG_MSG( "WM_IME_STARTCOMPOSITION (HKL=%x, wParam=%x, lParam=%x)\n", 
			g_hklCurrent, wParam, lParam );
		InitCompStringData();
		*trapped = true;
		break;
    ...
    case WM_IME_COMPOSITION:
		{
			BW_IME_DEBUG_MSG( "WM_IME_COMPOSITION (HKL=%x, wParam=%x, lParam=%x)\n", 
				g_hklCurrent, wParam, lParam );
			LONG lRet;
			...
			if (NULL == (himc = _ImmGetContext(hWnd)))
			{
		        ...
		        if ( lParam & GCS_COMPSTR )
		        {
					...
					if ( GETLANG() == LANG_CHT && !GetImeId() )
					{
						...
						if (g_szCompositionString[0] != '\0')
						{
							// BigWorld: our presentation layer will cope, so just copy everything across.
							wcscpy_s(g_szReadingString, COUNTOF(g_szReadingString), szCompStr);
							memset(&g_szCompositionString, 0, sizeof(g_szCompositionString));
							g_bReadingWindow = true;
							GetReadingWindowOrientation( 0 );
							g_szCompositionString[0] = TEXT(' '); // BigWorld: stick a dummy space in here.
						}
						else
							g_dwCount = 0;
					}
					...
					// BigWorld: make sure the reading window won't get stuck on.
					if (!g_szCompositionString[0])
					{
						g_szReadingString[0] = NULL;
    ...
    case WM_IME_ENDCOMPOSITION:
		BW_IME_DEBUG_MSG( "WM_IME_ENDCOMPOSITION (HKL=%x, wParam=%x, lParam=%x)\n",
			g_hklCurrent, wParam, lParam );
    ...
    case WM_IME_NOTIFY:
        switch (wParam)
        {
			case IMN_SETCONVERSIONMODE:
				{
					BW_IME_DEBUG_MSG( "IMN_SETCONVERSIONMODE (HKL=%x, wParam=%x, lParam=%x)\n", 
						g_hklCurrent, wParam, lParam );
					// Disable CHT IME software keyboard.
					static bool bNoReentrance = false;
					if (LANG_CHT == GETLANG() && !bNoReentrance)
					{
						// BigWorld: Don't disable soft keyboard!
						/*
						bNoReentrance = true;
			...
			case IMN_SETOPENSTATUS:
				BW_IME_DEBUG_MSG( "IMN_SETOPENSTATUS (bUILessMode=%d, HKL=%x, wParam=%x, lParam=%x)\n", 
					IsUILessMode(), g_hklCurrent, wParam, lParam );
				if (IsUILessMode())
					break;
				CheckToggleState();
				break;
			...
			case IMN_OPENCANDIDATE:
				BW_IME_DEBUG_MSG( "IMN_OPENCANDIDATE (bUILessMode=%d, HKL=%x, lCandidateList=%x)\n", 
					IsUILessMode(), g_hklCurrent, lParam );
			case IMN_CHANGECANDIDATE:

				if (wParam != IMN_OPENCANDIDATE)
				{
					BW_IME_DEBUG_MSG( "IMN_CHANGECANDIDATE (bUILessMode=%d, HKL=%x, lCandidateList=%x)\n", 
						IsUILessMode(), g_hklCurrent, lParam );
				}
				...
				}
				{
					...
					*trapped = true;
					...
					// BigWorld: Reading windows and candidate windows can co-exist?
					//g_bReadingWindow = false;
					dwIndex = 0;
					...
					if ( dwBufLen )
					{
						...
						if (GETLANG() == LANG_CHS && GetImeId())
						{
							...
							// BigWorld: We've calculated the correct page size above, but now use the given offset
							// to read out the candidate list strings.
							startOfPage = lpCandList->dwPageStart;
						}
						else
						{
							g_uCandPageSize = min( lpCandList->dwPageSize, DWORD(MAX_CANDLIST) );
							if (g_uCandPageSize > 0) // BigWorld: avoid division by zero
							{
								startOfPage = IsUILessMode() ? lpCandList->dwPageStart : ( g_dwSelection / g_uCandPageSize ) * g_uCandPageSize;
							}
							else
							{
								startOfPage = 0;
			...
			case IMN_CLOSECANDIDATE:
				BW_IME_DEBUG_MSG( "IMN_CLOSECANDIDATE (bUILessMode=%d, HKL=%x)\n", 
					IsUILessMode(), g_hklCurrent );
				if ( IsUILessMode() )
				{
					break;
				}
			...
			case IMN_PRIVATE:
				{
					// BigWorld: candidate lists and reading windows can coexist...?
					//if ( !g_bCandList )
					{
						GetReadingString( hWnd );

						if (!g_szReadingString[0])
						{
							g_bReadingWindow = false;
						}
					}
					...
					BW_IME_DEBUG_MSG( "IMN_PRIVATE (dwID=%x, HKL=%x, lParam=%x)\n", 
						dwId, g_hklCurrent, lParam );
			...
			default:
				BW_IME_DEBUG_MSG( "Unhandled WM_NOTIFY (HKL=%x, wParam=%x, lParam=%x)\n", 
					g_hklCurrent, wParam, lParam );
				// BigWorld: Setting this to true breaks some IME's such as Japanese on XP.
				//*trapped = true;
				break;
		}
		break;
...
static void GetReadingString(HWND hWnd)
{
	...
	// BigWorld: Why are we overwriting candidate list with reading string?????
	if ( unicode )
	{
		int i;
	...
	*/
	...
	wcsncpy_s( g_szReadingString, ARRAY_SIZE(g_szReadingString), wstr, 16 );
	if ( dwlen >= ARRAY_SIZE(g_szReadingString))
		dwlen = ARRAY_SIZE(g_szReadingString)-1;
	g_szReadingString[dwlen] = 0;
	/*
...
void ImeUi_FinalizeString(bool bSend)
{
	...
	CloseCandidateList();
	// BigWorld: make sure the reading string doesn't get stuck on in some languages
	g_bReadingWindow = false;
	g_szReadingString[0] = NULL;
	bProcessing = false;
	return;
...
static void OnInputLangChange()
{
	...
	// BigWorld: This causes the language bar to dissappear.
	/*
	HWND hwndImeDef = _ImmGetDefaultIMEWnd(g_hwndCurr);
	if ( hwndImeDef )
	{
...
{
	BW_IME_DEBUG_MSG( "CTsfUiLessMode::SetupSinks\n" );
    ...
    if (hr != S_OK)
    {
		BW_IME_DEBUG_MSG( "CTsfUiLessMode::SetupSinks CoCreateInstance Fail (hr=%d)\n", hr );
        return FALSE;
    }

    // ready to start interacting
	TfClientId cid;	// not used
	hr = m_tm->ActivateEx(&cid, TF_TMAE_UIELEMENTENABLEDONLY);
    if (FAILED(hr))
    {
		BW_IME_DEBUG_MSG( "CTsfUiLessMode::SetupSinks ActivateEx Fail (hr=%d)\n", hr );
        return FALSE;
    }
		...
		if (SUCCEEDED(hr = m_tm->QueryInterface(__uuidof(ITfSource), (void **)&srcTm)))
		{
			...
			if (SUCCEEDED(hr = srcTm->AdviseSink(__uuidof(ITfUIElementSink), (ITfUIElementSink*)m_TsfSink, &m_dwUIElementSinkCookie)))
			{
				...
				if (SUCCEEDED(hr = srcTm->AdviseSink(__uuidof(ITfInputProcessorProfileActivationSink), (ITfInputProcessorProfileActivationSink*)m_TsfSink, &m_dwAlpnSinkCookie)))
				{
					...
					{
						bRc = TRUE;
						BW_IME_DEBUG_MSG( "CTsfUiLessMode::SetupSinks - Sinks have been setup.\n" );
					}
				}
			}
			srcTm->Release();
...
void CTsfUiLessMode::ReleaseSinks()
{
	...
	BW_IME_DEBUG_MSG( "CTsfUiLessMode::ReleaseSinks\n" );
	...
	if ( m_tm && SUCCEEDED(m_tm->QueryInterface(__uuidof(ITfSource), (void **)&source)))
	{
		...
		BW_IME_DEBUG_MSG( "CTsfUiLessMode::ReleaseSinks - Sinks have been released.\n" );
	}	
}
...
{
	BW_IME_DEBUG_MSG( "CUIElementSink::BeginUIElement (dwUIElementId=%d, *pbShow=%d)\n", dwUIElementId, *pbShow );

    ITfUIElement *pElement = GetUIElement(dwUIElementId);
    if (!pElement)
	{
		BW_IME_DEBUG_MSG( "CUIElementSink::BeginUIElement - GetUIElement returned NULL\n" );
        return E_INVALIDARG;
	}

#ifdef BW_ENABLE_IME_DEBUGGING
	BSTR desc;
	if ( SUCCEEDED(pElement->GetDescription( &desc )) )
	{
		BW_IME_DEBUG_MSG( "CUIElementSink::BeginUIElement: pElement->GetDescription=%s)\n", bw_wtoutf8( desc ).c_str() );
		SysFreeString(desc);
	}
#endif
...
{
	BW_IME_DEBUG_MSG( "CUIElementSink::UpdateUIElement (dwUIElementId=%d)\n", dwUIElementId );
    ITfUIElement *pElement = GetUIElement(dwUIElementId);
    if (!pElement)
	{
		BW_IME_DEBUG_MSG( "CUIElementSink::UpdateUIElement - GetUIElement returned NULL\n" );
        return E_INVALIDARG;
	}
	...
	if ( SUCCEEDED(pElement->GetDescription( &desc )) )
	{
		BW_IME_DEBUG_MSG( "CUIElementSink::UpdateUIElement: pElement->GetDescription=%s)\n", bw_wtoutf8( desc ).c_str() );
		SysFreeString(desc);
	}
#endif
...
{
    BW_IME_DEBUG_MSG( "CUIElementSink::EndUIElement (dwUIElementId=%d)\n", dwUIElementId );
    ITfUIElement *pElement = GetUIElement(dwUIElementId);
    if (!pElement)
	{
		BW_IME_DEBUG_MSG( "CUIElementSink::EndUIElement - GetUIElement returned NULL\n" );
        return E_INVALIDARG;
	}
	...
	if ( SUCCEEDED(pElement->GetDescription( &desc )) )
	{
		BW_IME_DEBUG_MSG( "CUIElementSink::EndUIElement: pElement->GetDescription=%s)\n", bw_wtoutf8( desc ).c_str() );
		SysFreeString(desc);
	}
#endif
...
STDAPI CTsfUiLessMode::CUIElementSink::OnActivated(DWORD dwProfileType, LANGID langid, REFCLSID clsid, REFGUID catid,
		REFGUID guidProfile, HKL hkl, DWORD dwFlags)
{
	BW_IME_DEBUG_MSG( "CUIElementSink::OnActivated - dwProfileType=%x, langid=%x, clasid=%x, catid=%x, guidProfile=%x, hkl=%x, dwFlags=%x\n", 
		dwProfileType, langid, clsid, catid, guidProfile, hkl, dwFlags );
...
{
	BW_IME_DEBUG_MSG( "CUIElementSink::OnChange - rguid=%d\n", rguid );
	UpdateImeState();
    return S_OK;
}
...
void CTsfUiLessMode::MakeReadingInformationString(ITfReadingInformationUIElement* preading)
{
	...
	BW_IME_DEBUG_MSG( "CTsfUiLessMode::MakeReadingInformationString (dwFlags=0x%x, cchMax=%d, uErrorIndex=%d, fVertical=%d)\n",
		dwFlags, cchMax, uErrorIndex, fVertical );
	...
	if (SUCCEEDED(preading->GetString(&bstr)))
	{
		...
		{
			g_szReadingString[0] = NULL; // BigWorld: make sure reading string doesn't hang around.
			BW_IME_DEBUG_MSG( "CTsfUiLessMode::MakeReadingInformationString - preading->GetString returned NULL string.\n" );
		}
	}
}
...
void CTsfUiLessMode::MakeCandidateStrings(ITfCandidateListUIElement* pcandidate)
{
	...
	g_uCandPageSize = min(dwPageSize, DWORD(MAX_CANDLIST));
...
TCHAR* ImeUi_GetReadingString()
{
	return g_szReadingString;
}
...
{
	if (!g_szCompositionString[0] && g_dwCount > 0)
	{
		return TEXT(" "); // BigWorld: If we have candidates, make sure we always have a composition.
	}
	else
	{
