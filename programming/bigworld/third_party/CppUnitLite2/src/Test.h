...
#define ASSERT_WITH_MESSAGE( condition, message )							\
    do {																	\
		const bool testedCondition = (condition);							\
        try {																\
            if (!(testedCondition))											\
			{																\
				result_.AddFailure(											\
...
#define RETURN_ON_FAIL_CHECK(condition)                                                     \
    do {                                                                               \
		const bool testedCondition = (condition);									   \
        try {                                                                          \
            if (!(testedCondition))                                                    \
...
///Declares a functor operator within which the CHECK* macros can be used.
#define TEST_SUB_FUNCTOR_OPERATOR \
	void operator()(TestResult & result_, const char * m_name)

/**
 *	Calls a functor to run tests. The function operator should have been
 *	declared using TEST_SUB_FUNCTOR_OPERATOR.
 */
#define TEST_SUB_FUNCTOR(functor) (functor)(result_, m_name)

/**
 *	Same as TEST_SUB_FUNCTOR, but an extra label will be appended to the test
 *	name. Useful for identifying which sub-call failed.
 */
#define TEST_SUB_FUNCTOR_NAMED(functor, extraLabel) \
	do\
	{\
		std::ostringstream testInternalNameStr;\
		testInternalNameStr << m_name << ": " << extraLabel;\
		const std::string testInternalName = testInternalNameStr.str();\
		functor(result_, testInternalName.c_str());\
	} while (0)
