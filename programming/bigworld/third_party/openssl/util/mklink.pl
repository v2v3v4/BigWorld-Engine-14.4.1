...
# BigWorld: Added this section to support building from a Windows mount
#           under Linux. ie: symlink( "","") will work, however linking
#           in the local directory will fail.
if ($symlink_exists) {
    $new_file=$0 . "_symlink_test";
    $symlink_exists=symlink( $0, $new_file );
	$symlink_readable=(`wc -l $new_file &> /dev/null` == `wc -l $0`);
    if ($symlink_exists) {
        unlink( $new_file );
    }
	$symlink_exists = $symlink_readable;
}
