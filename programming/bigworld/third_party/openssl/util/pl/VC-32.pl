...
# BigWorld local modification to separate 64-bit output files
$out_def ="out";
			$out_def.='32'			if ($FLAVOR =~ /WIN32/);
			$out_def.='64'			if ($FLAVOR =~ /WIN64/);
			$out_def.="dll"			if ($shlib);
			$out_def.='_$(TARGETCPU)'	if ($FLAVOR =~ /CE/);
$tmp_def ="tmp";
			$tmp_def.='32'			if ($FLAVOR =~ /WIN32/);
			$tmp_def.='64'			if ($FLAVOR =~ /WIN64/);
			$tmp_def.="dll"			if ($shlib);
			$tmp_def.='_$(TARGETCPU)'	if ($FLAVOR =~ /CE/);
$inc_def="inc";
			$inc_def.='32'			if ($FLAVOR =~ /WIN32/);
			$inc_def.='64'			if ($FLAVOR =~ /WIN64/);

if ($debug)
	{
	$cflags=$dbg_cflags.$base_cflags;
	# BigWorld local modification for bug 31566
	# generate PDB only when doing a debug build
	$app_cflag.=" /Z7";
	$lib_cflag.=" /Z7";
	$lflags.=" /debug";
	}
