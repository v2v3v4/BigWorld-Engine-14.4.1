...
{
public:
	inline rcContext(bool state = true) : m_logEnabled(state), m_timerEnabled(state), m_stopped(false) {}
	virtual ~rcContext() {}
	...
	// Is the task stopped?
	bool stopped() const volatile { return m_stopped; }
	void stop() volatile { m_stopped = true; }
protected:
...
//	nv - (in) vertex count
//	areas - (out) array of triangle area types
void rcMarkWalkableTriangles(rcContext* ctx, const float walkableSlopeAngle, const float* verts, int nt,
							 unsigned char* areas); 
...
//	nv - (in) vertex count
//	areas - (out) array of triangle area types
void rcClearUnwalkableTriangles(rcContext* ctx, const float walkableSlopeAngle, const float* verts, int nt,
							 unsigned char* areas); 
