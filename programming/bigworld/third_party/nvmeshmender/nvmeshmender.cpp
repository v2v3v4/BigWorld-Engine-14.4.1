...
void MeshMender::ProcessNormals(TriangleList& possibleNeighbors,
								BW::vector< Vertex >&    theVerts,
								BW::vector< unsigned int >& mappingNewToOldVert,
								D3DXVECTOR3 workingPosition)
{
		...
		BW::vector<D3DXVECTOR3> groupNormalVectors;
		...
		BW::set<size_t> otherGroupsIndices;
		for( unsigned int i = 0; i < neighborGroups.size(); ++i )
		{
			TriangleList& curGroup = neighborGroups[ i ];
			BW::set<size_t> thisGroupIndices;
			...
			for(BW::set<size_t>::iterator it = thisGroupIndices.begin(); it!= thisGroupIndices.end() ; ++it)
			{
								...
								D3DXVECTOR3 workingPosition)
{
		...
		BW::vector<D3DXVECTOR3> groupTangentVectors;
								...
								D3DXVECTOR3 workingPosition)
{
		...
		BW::vector<D3DXVECTOR3> groupBinormalVectors;
				  ...
				  const CylindricalFixOption fixCylindricalWrapping)
{
	...
	// RA: NOTE: moving this initialisation out of the SetUpData(..) function
	// to avoid the errors in the FixCylindricalWrapping function 
		...
		++vert)
	{
		...
		if (computeNormals != DONT_CALCULATE_NORMALS_OR_BINORMALS)
		{
				   ...
				   BW::vector< Vertex >& theVerts)
{
	...
	*neighbor2 = NULL;
	...
	BW::vector<Triangle*> theNeighbors;
				  ...
				  const NormalCalcOption computeNormals)
{
	...
	//RA: moved outside this function
	//initialize the mapping
...
void MeshMender::UpdateTheIndicesWithFinalIndices(BW::vector< unsigned int >& theIndices )
{
	...
	{
		theIndices[oIndex+0] = (unsigned int)m_Triangles[i].indices[0];
		theIndices[oIndex+1] = (unsigned int)m_Triangles[i].indices[1];
		theIndices[oIndex+2] = (unsigned int)m_Triangles[i].indices[2];
		oIndex+=3;
	}
}


void MeshMender::FixCylindricalWrapping(	BW::vector< Vertex >& theVerts , 
								BW::vector< unsigned int >& theIndices,
								BW::vector< unsigned int >& mappingNewToOldVert)
{
	...
	{
		//for each triangle
		BW::set<unsigned int> alreadyDuped;
			...
			if( sBegin <= 1.0f  && sEnd <= 1.0f  && sBegin >= 0.0f  && sEnd >= 0.0f  )
			{
				...
				if( fabsf(sBegin - sEnd) > 0.5f )
				{
					...
					if(alreadyDuped.find(theOneToDupe) == alreadyDuped.end())
					{
						...
						theDupe.s += 1.0f;
						theIndices[index + theOneToDupe] = (unsigned int)theVerts.size();
						...
						BW::vector< unsigned int >& mappingNewToOldVert)
{
	...
	else
	{
		...
		mappingNewToOldVert.push_back( (unsigned int)oldIndex );
	}
}
